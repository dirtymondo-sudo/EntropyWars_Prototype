...
## 2026-09-11 — Ego Death / Time Rewind

Two Ego Death timer sites and four Time Rewind timer sites now use _fxDelay, including the repeated ghost-frame timer. These are emissions, not resource cleanup. Generation cancellation and delayed spells-category gates are regression-tested. Psychosis has no direct timer; Rune Sphere and Skull use _sigRunOwned, as do its tunnel/mandala dependencies. Direct particles are synchronous; texture/cache and full transitive acceptance remain separate. Earlier direct-timer line tables are historical snapshots; the Ego Death / Time Rewind entries are superseded here. Next: Merkaba and subsequent direct signature timers, then VFX-04.
