...
## THE SPELL TIERS — no secondary job, no branches, Tier I–IV cost 1–4 SP, 16 SP over 7 slots (2026-09-24, local delivery)
mondo: "get rid of units having a secondary job ... ditch the branches ... the node rung level that a spell is on
becomes its tier ... Tier I–IV cost 1, 2, 3, 4 SP ... still 7 spell slots ... total SP = 16 ... any spell can be
selected anywhere." THE RULES (data.js "THE TIERS" block, after the Freelancer pools):
- **Tier = rung.** `spellTierOf(id)` = `buildTreeRingIndex` ring + 1 (a shared spell takes its lowest rung — the
  same read that prices MP 25/50/75/100, so SP and MP always agree); off-row spells fall back to their MP.
  `spellSpCost` = tier (`SPELL_TIER_SP`), the basic attack is 0. `SPELL_SP_MAX = 16`, `SPELL_SLOT_MAX` stays 7.
- **The pool** (`unitSpellPoolParts(race, cls)` → race / job / borrowRace / borrowJob; `unitSpellPool`): the race
  row (EVERY twin alternate — both may be equipped now, the one-of-two fork is gone) + the job's four. A
  **Freelancer** has no job row and no sockets: it borrows ANY ability from `flRacePool` / `flWildcardPool` (story
  scope still limits those to owned vessels), bounded only by 7 slots and 16 SP.
- **Legality** `isTreeLoadoutLegal(race, cls, sec, ids)`: ≤ 7, no dups, ≤ 16 SP, every id in the pool and unsealed
  (Clash). No adjacency, no climbing. `spellAddVerdict(...)` → `{ ok, reason: ok|dup|pool|sealed|slots|sp, note }`
  is the ONE verdict every UI prints. `treeLegalSubset` repairs stale saves / hacked online kits: walk the list in
  order, keep what fits, SKIP (never truncate at) the rest — never throws. `trimSpellIdsToSlotBudget` enforces SP too.
- **Online**: online.js unchanged — the host already runs `treeLegalSubset` on a guest's kit and map.js
  `createUnit` runs it too, so the rules are host-authoritative with no new relay.
- **Defaults / random**: default = the job's four + the race's first two rungs (13 SP, 6 slots; a Freelancer its
  race row). `buildTreeLegalLoadout` = random affordable picks until nothing fits (a Freelancer weights its own row
  6:1 over borrows), up to 3 retries so the kit carries a damage spell.
- **Secondary job RETIRED**: `applySecondaryJob` / `aiPickSecondaryJob` are no-ops (no stat bonus, no borrowed
  spell), the level-15 milestone + its dialog trigger and the HQ 'SECONDARY JOB UNLOCKED' milestone are gone, the
  builder's subclass pill / picker / bar are gone, `meta.secondaryJob` is deleted on the next read/write
  (party-builder, state.js last-party, hqPartySetSpells). `SECONDARY_JOB_LEVEL` stays as a retired constant; ui.js's
  `secondaryJobPick` dialog is unreachable (nothing sets `_pendingSecondaryJobPick`).
- Removed from data.js: FL_FIXED, FL_SOCKET_TIERS/POOL, flSocketPool, buildFreelancerTree, the reachability /
  sealed-tree helpers, hqPartyTreePath / NodeState / DropIds, HQ_CIRCUIT_LANES. `getTreeEdges()` returns `[]`;
  `buildUnitSpellTree` is a shim (`nodes` R1–4/P1–4, `edges: []`, `alts`, `tiers`). The balance export's tree
  shape is tier counts (`R·P·T####`).
- UI: see ui-menus-audio.md "THE TIER RACK". Tests: spell-tree-ux.test.js (rewritten), content-schema tests
  "twin nodes", "tier legality", "Freelancer borrows"; `npm test` 1970 / 0 / 68 skipped.
