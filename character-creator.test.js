...
test('rev 8 — the mirror: the officer look on the profile, the chair’s look mode, the walker, the card’s bust, the creator screens render', async () => {
    ...
    for (const s of ["_mountReactCreator: '_unmountReactCreator'", "_mountReactCreator: 'CHARACTER CREATOR'", "if (mode === 'look' && typeof window.hqLook === 'function')", "if (getCharacterAppearanceModel(r, lk.gender, lk.appearance)) return { race: r, gender: lk.gender, appearance: lk.appearance };", "btn('look', 'YOUR OWN LOOK'", 'data-fn="_mountReactCreator"', 'window._hqRefreshAvatar = function ()']) assert.ok(mp.includes(s), 'map.js has ' + s);
