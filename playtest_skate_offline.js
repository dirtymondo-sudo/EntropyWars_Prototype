// playtest_skate_offline.js — SKATEBOARDING probe (2026-09-17, rev 3): the gun probe's mirror (every repo GLB served from disk — the animation
// libraries, the creator base, the skateboard at the repo root) so the walker is a POSED rig on the real deck. Same steps as
// playtest_gun_offline.js ([{eval}, {wait}, {teleport}, {shot}, {keydown|keyup}, {sample}]); the ride through hq.skate(true) + hq.dev.press.
//   node playtest_skate_offline.js training '[{"eval":"ThreeRenderer.hq.skate(true)"},{"keydown":"w"},{"wait":3000},{"shot":"push"}]' tag
// Needs the server (npm start) + npm i --no-save three@0.128.0 react@18 react-dom@18 playwright three.meshline. Shots → shots/skate/.
const fs = require('fs'); const path = require('path'); const zlib = require('zlib');
const REPO = __dirname;
const { chromium } = require(path.join(REPO, 'node_modules/playwright'));
const OUT = path.join(REPO, 'shots/skate'); fs.mkdirSync(OUT, { recursive: true });
...
(async () => {
  ...
  await page.evaluate(() => { window.EW_HQ_NO_POST = true; window.EW_DISABLE_CAST = true; window.EW_HQ_VARIANT = 'none'; window.EW_HQ_AVATAR = { race: 'homosapien', gender: 'male', appearance: (typeof normalizeCharacterAppearance === 'function') ? normalizeCharacterAppearance({ outfit: 'suit', bottoms: 'trousers', feet: 'boots' }) : null }; });
