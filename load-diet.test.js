...
test('the menu warms the walker\'s rig ONLY (the model queue); Play warms the room', () => {
    assert.ok(/window\._hqWarmArrival\(\{ avatarOnly: true \}\); \} catch \(e\) \{\} return; \}/.test(map), '_hqWarmArrivalSoon is avatar-only');
