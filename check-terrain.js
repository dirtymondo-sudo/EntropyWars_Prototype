...
// the walker's own rule (data.js hqTerrainReach) AND that nothing traps (2026-09-17: hqTerrainTraps —
// every cell the walker can fall into returns to a door; the rescue ramps the compiler cut are listed). Usage:
...
for (const id of ids) {
    ...
    const rec = { id, w: info.S.w, d: info.S.d, res: info.res, grid: info.nx + '×' + info.nz, lo: +lo.toFixed(2), hi: +hi.toFixed(2), reach: reach.size, doors: landings.map(L => L.id + '@' + L.y), unreached, trees: info.trees.length, scatter: info.scatter.length, fluids: info.fluids.length, walls: info.walls.length, bridges: (info.bridges || []).length,
                  plan: info.gen ? { kind: info.gen.kind, open: +info.gen.open.toFixed(2), carved: info.gen.carved, sealed: info.gen.sealed, wallH: info.gen.wallH, thicket: (info.thicket || []).length, planWalls: (info.planWalls || []).length, deadEnds: (info.genPlan && info.genPlan.deadEnds) ? info.genPlan.deadEnds.length : 0, alleys: info.gen.alleys || 0 } : null, traps, rescues };
    ...
    console.log('   trees ' + rec.trees + ' · scatter ' + rec.scatter + ' · fluids ' + rec.fluids + ' · walls ' + rec.walls + (rec.bridges ? ' · bridges ' + rec.bridges : '') + ' · climbs ' + ((info.climbs || []).length) + (rec.plan ? '   PLAN ' + rec.plan.kind + ': open ' + Math.round(rec.plan.open * 100) + '%, corridors carved ' + rec.plan.carved + ', solid ' + rec.plan.wallH + ' m' + (rec.plan.thicket ? ', thicket ' + rec.plan.thicket + ' trees' : '') + (rec.plan.planWalls ? ', traced walls ' + rec.plan.planWalls : '') + (rec.plan.alleys ? ', alleys ' + rec.plan.alleys : '') : '   (no floor plan: the box is the floor)'));
    console.log('   RETURN: ' + (traps.length ? 'TRAPS ' + traps.map(t => t.cells + ' cells @ ' + t.x + ',' + t.z + ' (' + t.y + ' m)').join(' · ') : 'nothing traps') + (rescues.length ? ' · rescue ramps cut ' + rescues.length + ' (' + rescues.map(q => q.y0 + '→' + q.y1 + '/' + q.len + ' m').join(', ') + ')' : ''));
