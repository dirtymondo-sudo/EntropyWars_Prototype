...
for (const id of ids) {
    ...
    console.log('   trees ' + rec.trees + ' · scatter ' + rec.scatter + ' · fluids ' + rec.fluids + ' · walls ' + rec.walls + (rec.plan ? '   PLAN ' + rec.plan.kind + ': open ' + Math.round(rec.plan.open * 100) + '%, corridors carved ' + rec.plan.carved + ', solid ' + rec.plan.wallH + ' m' + (rec.plan.thicket ? ', thicket ' + rec.plan.thicket + ' trees' : '') + (rec.plan.planWalls ? ', traced walls ' + rec.plan.planWalls : '') : '   (no floor plan: the box is the floor)'));
