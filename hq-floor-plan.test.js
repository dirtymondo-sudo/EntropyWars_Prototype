...
test('the sheet: thirteen of the fourteen terrain rooms carry a floor plan — the cave chambers cellular automata, the open woods clearings + corridors; the storm drain (a culvert) keeps its box', () => {
    ...
    for (const id of PLANNED) {
        ...
        if (room.shell.open) assert.equal(gen.kind, 'rooms', id + ': an open room grows a thicket, not rock');
...
test('THE MASK IS THE HEIGHT: every solid cell deep in the plan stands at least half the wall above the floor round it, every forced cell is open, the rim of a plan is solid, and the field is a wall to the walker by the slope rule alone', () => {
    for (const id of PLANNED) {
        ...
        /* the walker's rule: from open ground beside the solid, walk straight at it carrying the feet — the rise is never climbed (the slope refuses it) */
