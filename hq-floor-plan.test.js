...
test('the sheet: seventeen of the eighteen terrain rooms carry a floor plan — the cave chambers cellular automata, the open woods clearings + corridors; the storm drain (a culvert) keeps its box', heavy, () => {
    ...
    for (const id of PLANNED) {
        ...
        else if (gen.kind === 'ley') { assert.ok(info.genPlan && info.genPlan.lines.length >= 2 && info.genPlan.forks.length >= 6 && info.genPlan.chambers.length >= 4 && info.planWalls.length >= 40 && info.gen.solidMass && info.solidTop, id + ': a ley plan — lines, forks, chambers, traced walls, the mass'); }   // THE LEY LINES (2026-09-18)
...
test('THE ROOM LOOKS: every woods room, every cave chamber and the storm drain wear a look; five battle maps (and their Δs) carry one in env.look; a look is the overlay\'s shape', () => {
    ...
    for (const id of ROOMS.filter(id => id.includes('hollow_earth') && !HQ.rooms[id].area)) { /* THE AREAS (2026-09-18): THE INNER SUN stands under the sky */ assert.equal(HQ.rooms[id].shell.look, D.HQ_ROOM_LOOKS.cave, id); assert.ok(HQ.rooms[id].shell.fog && HQ.rooms[id].shell.fog.density > 0, id + ': the cave\'s own haze'); }
