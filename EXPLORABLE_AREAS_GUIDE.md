...
- 2026-09-26: **OPEN_WORLD_PLAN.md** written (nothing built) — ZONES of stitched parts on one ground (a room gets a FRAME in `DOOR_HQ.world`, a JOIN along a shared edge, a door that swings instead of loading), the stage that draws the current part and its joined neighbours, far shells for the weenies, a world clock, the map drawn from the frames, the BUILT rebuilds (the mall, Camelot, the Vatican, the bunker, the D.U.M.B.), the wells re-pointed, the buffer rooms merged. Read it before any room / area / link work from now on: a room the plan moves is built IN its frame. Its §8 rebuilds follow BUILT_ARCHITECTURE_PLAN §4 and this guide's §8 checklist.
