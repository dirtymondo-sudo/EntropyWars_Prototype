...
### 2026-09-19 — THE WORLD OVERVIEW: the directory reorganised round the hubs (local delivery)
The user, with a reference sheet: "make the map / directory more organized — the main hubs / nodes should be DOOR HQ, The Woods, The Estate (rename the ranch), The Cavern, The Deep, The D.U.M.B., etc." The directory opens on **THE WORLD** now (data.js `hqWorldOverviewGraph` / `hqWorldOverviewLayout` / `hqWorldOverview`, the block after `hqMapModel`): ONE node per PLACE — the building as a BLOCK with a band per floor (PH · 4 · 3 · 2 · M · G · B · H-WING, the elevator's stops + H-Wing; `hqWorldFloorOf(room)` = the floor a facility room hangs off), a big ringed node with a glyph per `DOOR_HQ.hubs` row, a small numbered node per lone site — and ONE octilinear transit line per pair of places (main = a door / a link / a lift, secondary = a seam that is not a door, secret = a draught, the ring's bay threads only for the place picked), in its route's ink. The positions are an AUTHORED table (`HQ_WORLD_L.slots` — tune it, never the code; a place the table does not name lands on a fallback ring and hq-map.test.js says so). The panel (map.js "THE WORLD OVERVIEW" block): the crumb `MAP | WORLD ▸ CENTRAL OVERVIEW ▸ ALL LOCATIONS`, the status line (AREAS CHARTED n/N · SECRETS found/total or ??? · CLICK A PLACE TWICE TO TRAVEL), the LOCATIONS rail on the left (the facility, THE HUBS, THE LOCATIONS — charted places as rows in their ink, the uncharted a count per group), THE KEY under the stage, a compass. ONE click = the place's card (rooms charted, GO ▸ its anchor, OPEN THE AREA MAP; the building's card lists the floors with GO), a SECOND click on the same place travels. **THE AREA sheet** = the room map (hqMapModel) filtered to one place (`_hqAreaModel`; a room outside it a plain question mark; the building keeps its own layout, every other place a radial tree round its anchor — `_hqAreaLayout`; a room's name drops the place's prefix). WORLD / THIS AREA tabs on the bar; the reveal keeps a memory per sheet (`door.hq.map.w / we / wbox` beside `n / e / box` + `area`), so switching sheets never replays a pop. THE ESTATE is the ranch's name everywhere the player reads it (the hub, the route, the corn fields' plate, the well's plates; the ids stay `ranch`). Tests: hq-map.test.js THE WORLD OVERVIEW (the graph, the slots, the floors, the edges, the model, the panel on both sheets, the two-click travel); hq-ranch's plate pins; hq-world's seams pin caught up with D2's two gutters (pre-existing red on main). UNSEEN LIVE (RULE #1c): the sheet at the game's fonts and theme (the static render used the stylesheet alone), the two-click feel, the reveal on the world sheet, the rail at 860 px.

### 2026-09-19 — THE POPULATION + THE ROUNDS: every area inhabited by its own, and they walk (local delivery)
The user: "each area inhabited by the proper units — they already have map tags / terrain preferences; Disaster City and DOOR HQ the most diverse; make them walk in routed loops, even loops between doors or areas — make the game feel more alive." Data (data.js, the block before THE ENCOUNTER): `HQ_POPULATION_RULES`, `hqSiteResidents` (a site's people by the tags the game already keeps — POINT_OF_ENTRY natives first, then the races whose `RACE_PROFILES.biomes` meet the site's `EW_MAP_META.biomes`, then the neighbouring sites' natives, then the sector's), `hqRosterRaces`, `hqRoomFloorM2`, `hqRoomPopulation` (the ONE read: kind wild / city / facility, the pool, the seeded draw of extra walkers sized by the floor — Disaster City `cityMul` up to 12 a street, the hall `hqMul` up to 9, the sewers their own list), `hqSpotRoams`. Renderer (three-renderer.js "THE ROUNDS", before SKATEBOARDING): the nav lattice off the walker's own `_hqSurface` with the people left out of the furniture, built over frames; A* with no corner cutting and the walker's step both ways; the stops (doors, counters, spots, the spawn, free cells); the loop; the EXIT through a door that leads somewhere (the leaf swings, the walker is away, then back in by another door); the TRAVELLER LEDGER (the far room spawns the walker at the door it left by, walking in, when the officer follows); yielding; the pause freeze. Two hall agents `patrol`. `npm test` runs `hq-population.test.js` (7). Files: data.js (R2 + Render), three-renderer.js (R2), index.html (Render, `20260919-rounds-01-cors`), the test + the docs (repo). Eyeball first: the hall (nine walkers + two patrols on their loops, the thresholds swinging as people pass into the bays), THE STREETS (twelve on the sidewalks — the lattice takes a second or two to land, they stand until it does), a native leaving its spot and coming back, a walker vanishing through a door and the same race walking in ahead of you when you follow it. Tune `HQ_POPULATION_RULES` (perM2 / cityMax / walkSpeed / pauseMs / awayMs), never the code.

**2026-09-19 — THE LOADING SCREEN + THE SURVEY (local delivery).** The freeze after the door: map.js `_hqEnter` built the room in the task that showed the load card (never painted) and the finds warm compiled every tape room's floor plan on idle — 3–31 s each on the main thread. Now: the build is deferred past a paint (`_hqDeferBuild`), the old room holds under the card on a walk (`hq.hold`), the floor plans compile in a Web Worker running data.js (`hqTerrainWorkerServe` / `hqTerrainPlain` / `hqTerrainAdopt`; the room being entered first, the rest warmed nearest-first), the card's note is live (the survey's clock, the GLB files in flight), and the walker's rig + the arrival room's models start downloading behind the main menu (`_hqWarmArrival`). Eyeball first: the card on Play (it should PAINT before the door beat ends), a walk into Downtown's streets from a cold start (the full card with the clock, ~30 s off-thread, the hall still drawn underneath — no browser "page unresponsive"), a second entry (instant), the worker on a phone. Still the user's: the GLB weight (the hall ~100 MB; see CLAUDE.md's entry).

**2026-09-20 — THE PARTY'S QUICK ACTIONS + THE BAG + ROOM 911 · THE DISPENSARY + AUTO HEAL (local delivery).** The user's JRPG list: quick actions on the party cards (swap · relieve · every heal spell · the potions, on that member at once), AUTO HEAL (revive the down, heal the hurt — spells first, then the potions), an INVENTORY (THE BAG: `door.hq.bag`, filled by the pay caches (one potion each now), the shop and the pockets coming home; emptied by field uses and by THE POCKETS topped up at every launch), a SHOP room (Room 911 · THE DISPENSARY off the Medical Wing's north wall — THE HATCH sells `HQ_DISPENSARY.stock` at `shopPrice`, buys back at half; the gold moves through `ProfileSystem.spendGold` → `/api/economy/spend` on a server account), two field-only items (Revival Tonic, Elixir), the ITEMS command in the pause menu. Medical's COT is unchanged (the free inn); the quick bar walks you to it. Eyeball first: the quick strip under each card (the buttons wrap; a DOWN card greys its heals), AUTO HEAL's line after a hurt encounter, the ITEMS sheet, the hatch (BUY 1 / BUY 5 / SELL 1 and the gold line), the pharmacist seated behind the exec-desk counter (her chair is the `computer_chair_grey` at z −2.1 — if she floats or sits in the desk, `agents[0].z` is the edit), the wing's new door at x 2.2 between the ward's and the corner. CLAUDE.md has the rules. Tests: hq-party (14), hq-suites (the SUITES table), doorhq (the room's light + leaf).

### 2026-09-20 — THE EARNED DOORS + THE ROSTER LOCK (local delivery)
No bay threshold stands until its site is STABILIZED (every win condition filed from inside the
site) — then Otto builds it (the chime + the toast on the next arrival). Room 64 is the exception
(its hall door, its RANGE console with every site and the whole roster). The map hides the
unbuilt thresholds like secret doors; the star chart names only charted / earned stars; the hall's
bay panels list built thresholds; DISPATCH deals earned sites. The roster is locked to what the
