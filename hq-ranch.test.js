...
test('THE HUBS: every hub names a room that exists and gathers its rooms; the graph marks the anchor and the members; the model carries them; map.js draws the ring and the halo', () => {
    const H = HQ.hubs;
    assert.deepEqual(Object.keys(H).sort().join(','), 'cavern,city,deep,divine,dumb,hq,kingdom,ranch,underworld,woods');   // + THE UNDERWORLD (2026-09-18) + THE DEEP (2026-09-18) (2026-09-18: a hub that claims its rooms BY ID)
    ...
    assert.equal(M.nodes.filter(n => n.hub).length, 10, 'ten hubs drawn (THE UNDERWORLD, THE DEEP — 2026-09-18)');
...
test('THE PLATE READS THE ROOM THROUGH THE DOOR: every room door wears the label of the room its action lands in (a bay door keeps its segment), the entry part for a bypassed site, and a link door the far room\'s own', () => {
    ...
    assert.equal(at('site_prebuilt_fairy_forest', 'link_fairy_camelot').label, 'CAMELOT · THE OUTER WARD', 'a link door reads the part, not the site');
    assert.equal(at('site_prebuilt_lodge', 'link_ranch_lodge').label, 'THE RANCH · THE CORN FIELDS');
