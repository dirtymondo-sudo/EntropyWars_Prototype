...
| 25 | Phase 9 order (the user's 2026-09-15 brief: finds, complexes, the world graph, encounters, the door gun) | (the user's list order) | — | **REC:** 9.3 the WORLD GRAPH + suites (data only, answers the crowding) → 9.2 the COMPLEXES (the box-room gallery is the renderer stage that also pays for the third ring) → 9.1 the FINDS → 9.5 the DOOR GUN → 9.4 the ENCOUNTER. HQ plan Phase 9. |
| 26 | The hundred collectibles | "Rubik's Cubes or VHS tapes … like 100 of them"; the tapes are clues about the parents or anomaly evidence (the user's gifs) | — | **REC:** the TAPES are the hundred (`DOOR_TAPES`, each a clip on R2 `Assets/door/tapes/`, titles + captions the user's — A15), played at Room 360's projector on THE SHELF; plus TWELVE CUBES as the story's collectible (each a face short; the twelfth turns a tape to the parents'). Alt: tapes only. HQ plan 9.1. |
| 27 | The crowded hall | "too many rooms / doors crowding the hallways … a third ring floor would help" | 13 ground doors, 11 mezzanine today | **REC:** SUITES now (data: MEDICAL WING = Medical + the Padded Room + the Interrogation Room behind the 210° door; RECORDS WING = Records + the Clock Room + the Observatorium's stair behind 240°; EXECUTIVE = the Trophy Case + the Bureau behind one mezzanine door — −2 ground, −1 mezzanine) and THE GALLERY (a third ring inside the dome for the exploration doors) when the box-room gallery ships. HQ plan 9.3. **THE SUITES SHIPPED 2026-09-15 rev 9 (local): `medwing` behind 210° (Room 1111 + Room 5150 behind it + Room 1984), `recwing` behind 240° (Room 42 + Room 247; the Observatorium's stair stays Room 42's own), `execwing` behind 315° (Room 111 + the Bureau, whose GATE and CONTESTED number stay on its own door). Ground 13 → 11, mezzanine 11 → 10. The GALLERY still waits on the box gallery.** |
| 28 | The seams (site ⇄ site doors, and the seams that are NOT doors — the wardrobe into Camelot, the mirror, the well, the pool, the painting, the hearth, the screen, the train; HQ plan 9.3 `way`) | "a door that leads to this related map … the moon and the spaceship and Saturn … the bottom of the Flying Dutchman having a door to Atlantis" | A10's bays are the only grouping | **REC:** ten routes in the HQ plan 9.3 table — THE LUNAR ROUTE (Mars ⇄ Moon ⇄ the Spaceship's airlock ⇄ Saturn → the Singularity, one way), THE DEEP (the Dutchman's hold ⇄ Atlantis ⇄ Hollow Earth ⇄ Agartha ⇄ Shasta; Antarctica ⇄ the North Pole), THE DIVINE STAIR (Heaven ⇄ Olympus; the Vatican's archive ⇄ Heaven, its crypt ⇄ Hell; Hell ⇄ Hollow Earth), THE BASES (Area 51 ⇄ D.U.M.B. ⇄ CERN ⇄ the Backrooms), THE WOODS (the Haunted House's cellar ⇄ Skinwalker ⇄ the Grove ⇄ Fairy Forest), THE LEY LINE (Stonehenge ⇄ Göbekli ⇄ Giza ⇄ Babel ⇄ Technoticlan; Camelot ⇄ the Lodge), THE HIGHWAY (Nuketown ⇄ Downtown ⇄ the Strip ⇄ Cyberpunk; the Stadium ⇄ Downtown), THE WONDERLAND (the Looking-Glass ⇄ Flat Lands ⇄ the Backrooms). Every site on a route; the bays stay the customs face; a `room` door is never sector-gated (C-12). The user strikes / adds. **SHIPPED 2026-09-15 rev 6 (local): the first two NOT-A-DOOR seams — the wardrobe upstairs in the Haunted House ⇄ Camelot, the well in its cellar ⇄ Hollow Earth (`DOOR_HQ.ways` + two `way` link rows).** |
| 29 | The complexes' order | "Haunted house … its own little mansion with at least 2 floors and a basement or an attic … the urban maps could be a lot bigger … the spaceship or Flying Dutchman … different decks" | one generated room per site (7.2) | **REC:** the Haunted House first (hall · upstairs with the first TWO-FLOOR room · attic · cellar), then the Dutchman (gun deck · hold · cabin), the Spaceship (bridge · engineering · airlock), the urban block (the chapel, the casino, Downtown's lobby and subway). `siteRooms.backDoors` becomes an array; complex rooms wear `site` + `part`, no number. HQ plan 9.2. **The Haunted House SHIPPED LOCALLY 2026-09-15 (stage 1: four box rooms, no gallery yet); the rest await the user's order.** |
| 30 | The encounter's cost | "seeing an enemy roaming and attacking them and then the entire area you are in becomes a part of the 8×8 battle map" — "limit this to rooms not actually in the facility" | — | **REC:** wild rooms only (a site's board room + its complex parts; the facility is safe by construction); the party = THE LAST ROSTER, the config = the sticky one, no terminal; the first battle frame = the walker's own camera (the one new three-camera.js entry point); a loss = the ward (Medical's cot, the CONDITION line) and nothing else. Alts: hazard pay docked; a day's leave (`profile.door.leave`). VS-CPU only (RULE #2). HQ plan 9.4. |
| 31 | The door gun's issue | "a way to place 2 doors like a portal gun … puzzle solving … or place a door by a known safe area … like an escape rope" | the Door Agent race's Knock Knock (the same object on the board) | **REC:** THE PORTABLE THRESHOLD, DOOR issue, from the Quartermaster at KEYHOLDER (L2 — the door rank by name) for 24 Keys, the first door free at intake so the tutorial can teach it; placed on any walkable surface you can SEE (never a wall, never water), left click A / B / A again, Q holsters, persists for one visit. Alt: both doors on day one, the puzzles are the gate. HQ plan 9.5. **SHIPPED 2026-09-15 rev 13 (local): as the REC reads — the Quartermaster's door panel signs for it at KEYHOLDER for 24 Keys, F draws, the ghost over the walkable surface set, click A / B / A moves, Q holsters, the hop / the room change, cleared on the next arrival from Play (`door.hq.portal`; `?portal` forces the issue). The intake freebie and the tutorial lesson are not built. The alt is one line (`issued: true` by default).** |
| 32 | Skateboarding (optional) | "the round railings and rooms are perfect for grinding … very unnecessary but the game is really asking for it"; THAW / JSRF | — | **REC:** yes, as a walker MODE (HQ plan 9.8): B drops a deck found in the locker room, momentum + ollie + grinds on the rails the builders already make (the mezzanine's ring is a 130 m grind) + ramps + trick keys in the air, a combo line on the strip and one achievement, nothing on `state`, kill-switch `EW_HQ_NO_SKATE`. THE PARK RULE is in force now for every new room: a rail and a ramp per big room. The user rules yes / no / later. |
| 33 | The wells and the cave (HQ plan 9.3, rev 8) | "the well should be in the garden and lead to the cellar or a cave system or underground dungeon which can have different exits which lead to Hell, the DUMB, Agartha or Hollow Earth, etc. … keep the well in the haunted house cellar too but it should lead to the cave. There can be more than one well" | one well, cellar ⇄ Hollow Earth (rev 6) | **REC:** every well drops into ONE CAVE COMPLEX (Hollow Earth's parts, wild by construction): a WELL ROOM with a head per well (climb out of any), a gallery, four exit chambers — Hell's fissure, D.U.M.B.'s sixth side, Agartha's adit, the mouth onto Hollow Earth — an oubliette (tapes, 9.4's first roamer), route THE UNDERCROFT. Six wells: the garden (the facility's own way down, C-12's rule), the cellar re-pointed, Camelot, Skinwalker, Nuketown, Göbekli. Alt: the cave as its own numbered site. |
| 34 | The Rooftop (HQ plan 7.4 row R) | "DOOR HQ Rooftop (it's not what you'd expect)" | the penthouse is the top | **REC:** R above PH on the elevator's unlabelled button (the penthouse's gate reads through), an OPEN room on the dome's crown with the rail (the park rule); the TWIST is the user's (A15) — Claude's three drafts: the wrong sky over the roof (REC), the ground floor's plan painted on the concrete with one real door, the helipad as a board. |
| 35 | Tartaria + the Bermuda Triangle (HQ plan 7.7) | "Tartaria (lost advanced civilization), Bermuda Triangle (it's a right triangle)" | — | **REC:** 1812 · TARTARIA, Ancient — the buried fair city (the board one level down in a mud bowl, the world sinking past it), free-energy towers, star-fort near builder, the Bureau's first site RETCON notice, on THE LEY LINE + a `painting` seam from the Bureau; 345 · THE BERMUDA TRIANGLE, Hollow — the playable water a right triangle of tiles (`∠ 90°` on the plate), the `sea` moving map, Flight 19 as a flyover, on THE DEEP + the Infinity Pool's weir re-pointed to it. The Triangle first (its kit exists). |
| 36 | Water polo (HQ plan 9.9, super optional, after 9.8) | "there is a water polo git repo too, we can use stuff from that to add a water polo mini game in the natatorium" | Room 50M is a lap pool | **REC:** yes, later — port `dirtymondo-sudo/waterpolo`'s `src/sim` (headless, fixed-step, command-driven: the same source-of-truth rule) as ONE block in map.js (RULE #1), the natatorium's own builders + rigs draw it, a `polo` room variant (30 × 20 m) or short course in the lanes, the walker as a swimmer, VS CPU, viewer-local, the lifeguard's line + one achievement, kill-switch `EW_HQ_NO_POLO`; a second seat and the gamepad are stage 2. |
...
### 2026-09-15 (rev 13) — Phase 9.5 stage 1: THE DOOR GUN — THE PORTABLE THRESHOLD (HQ plan 9.5, local, not deployed)
- Row 31 built the way its REC read: the Quartermaster issues THE
  PORTABLE THRESHOLD at KEYHOLDER (L2, the door rank by name) for 24
  Keys; F draws it, a ghost frame follows the aim over any walkable
  surface the officer can SEE (never a wall, never water, never inside
  the furniture), LEFT CLICK places THRESHOLD A, then B, then A moves
  (the pair is always the last two), Q holsters; walk into one, step out
  of the other — in the same room a hop, across rooms the ordinary room
  change landing at the twin. The pair is one record on the card
  (`door.hq.portal`) and is cleared on the next arrival from Play: the
  escape rope is for one visit.
- The placed door wears the site's own threshold leaf in a site room
  (Room 13's parts wear the mansion's wooden door) and the plain coffee
  door in the facility — never a rank leaf. The seal is on the plate, the
  slot letter on the lintel, cyan for A and amber for B.
- Not taken from the REC: "the first door free at intake so the tutorial
  can teach it" — a lone door opens onto nothing, and Orientation has no
  door-gun tape yet. Owed: a lesson in 4.3 when the user wants one; the
  `hard` tapes on the wall cells are now reachable in principle and
  unseen in practice.
- Files: data.js, three-renderer.js, map.js, index.html, styles-base.css;
  hq-portal.test.js; HQ plan §9 2026-09-15 rev 13.
...
MEDICAL WING at 210° (the ward Room 1111, Room 5150 still behind the
ward's own cell door, Room 1984 at the far end of the corridor — Internal
Affairs was moved in with Medical during a reorganisation nobody can
produce the memo for), THE RECORDS WING at 240° (Room 42 keeps the file,
Room 247 keeps the time; they were put in one wing so they would stop
contradicting each other, and it has not worked) and THE EXECUTIVE SUITE
at 315° on the mezzanine (Room 111's case, and the Bureau's house door —
the second house door in one suite, which is the joke the Bureau has
already corrected once). The ground ring drops to eleven doors and the
mezzanine to ten; 225°, 255° and 290° are free wall.
Canon is untouched: every room keeps its number, its counters and its
cast, and the Bureau's GATEKEEPER + 24 Keys and `№ — CONTESTED` stay on
the Bureau's OWN door, so a recruit walks the suite, reads the notices
(still on the hall door too, at any rank — A7) and does not go in. The
lobbies are unnumbered, launch nothing, and their overheard lines are
Claude's drafts (A15). See HQ plan §9 2026-09-15 rev 9.
...
### 2026-09-15 (rev 3) — PHASE 9 PLANNED: THE WORLD (docs only)
The user's five-part brief (finds and a hundred tapes or cubes; sites as
whole buildings — the mansion, the decks, the urban block; a door in one
site that leads to a related site and a hall that is less crowded; a
roaming enemy you attack whose room becomes the board; two placeable
doors — puzzles and an escape rope) is written up as HQ plan Phase 9
with the data shapes (`DOOR_HQ.finds` / `DOOR_TAPES` / `DOOR_HQ.links`
/ `room.roamers` / `shell.gallery` / `door.hq.portal`), the renderer
stages, the tests and the assets. Seven decisions for the user are
Part C rows 25–31 (the order; tapes vs cubes; suites vs the gallery;
the ten routes; the complexes' order; the encounter's cost; the door
gun's issue). Story hooks noted, not written: the tapes' `parents` kind
...
the cellar goes down further than the house is tall, to Hollow Earth. Both
stand at the far end too (a wardrobe against the curtain wall, a well head
by the cave wall). A seam is a LINK (both ends in the world graph, the same
plate rule: ROOM № · the far site · the kind's function), voiced by its own
sound, entered by its own verb (CLIMB IN · CLIMB DOWN), and it lands you
facing AWAY from the object. `DOOR_HQ.ways` is the catalogue and the gate;
the other eight kinds (row 28) wait on their builders. The `why` / `note`
lines are Claude's DRAFT (A15). HQ plan §9 2026-09-15 rev 6;
hq-world.test.js. Complete files delivered for upload; nothing committed,
pushed or deployed.

### 2026-09-15 (rev 5) — Phase 9.2 stage 1: THE HAUNTED HOUSE COMPLEX (local, not deployed)
The first site that is several rooms, the user's own example (row 29).
Room 13's board room keeps the console, the marker and the way back to
the bay; THE FRONT DOOR on its north wall walks into THE HALL (the
staircase, the banister, the cellar door, the ghoul), UPSTAIRS (the
landing, four beds, THE WARDROBE that the wardrobe seam will use, the
ghost), THE ATTIC (one bulb, the trunk) and THE CELLAR (the furnace lit
by nobody, the racks, THE WELL, a box on trestles, the vampire). Every
plate reads 13; the register lists the house once; the rooms are WILD
(`hqRoomSite`) and the facility is safe by construction. The overheard
lines are Claude's DRAFT (A15). HQ plan §9 2026-09-15; hq-complex.test.js.
Complete files delivered for upload; nothing committed, pushed or deployed.


### 2026-09-15 (rev 4) — Phase 9 grows: the wardrobe into Camelot; skateboarding (docs only)
The user's two additions: seams that are not doors (a wardrobe in a
suburban house that opens on Camelot, and more entryways of that kind)
and, optional, skateboarding on the building's railings and ramps. HQ
plan 9.3 gained the `way` kinds with ten first seams; 9.8 is the ride
as a walker mode with THE PARK RULE for every new room. Part C row 28
updated, row 32 added. Nothing built.


### 2026-09-14 local / 2026-09-15 UTC — Phase 9 review; Lunar pilot implemented locally
The user requested review, fixes and a start on the additions. HQ plan
Phase 9 now separates a reversible connection pilot from unbuilt complex
rooms, suites and the later renderer/engine features. Its review corrects
the 38-site inventory, suite door reduction, reward persistence, encounter
mapping, portal landings and time-based skate physics. C25–32 remain
proposals except the technical pilot carried out in this request; no plot,
prices, twelve-cube reveal or optional skate reward was decided.

`data.js` generates matching Moon ⇄ Derelict ⇄ Saturn doors in the current
board rooms and accepts multiple site back doors. The airlock is still
unbuilt. Shared room numbers, bay exits, consoles and H-Wing are preserved.
See HQ plan §9 and HQ_VALIDATION.md for tests and limitations. Complete
files delivered for upload; nothing committed, pushed or deployed.

### 2026-09-15 (rev 8) — Phase 9 grows again: the wells and the cave, the Rooftop, Tartaria, the Bermuda Triangle, water polo (docs only)
The user's additions: the well belongs in the garden and every well
leads to one cave system with exits to Hell, D.U.M.B., Agartha and
Hollow Earth (the cellar's well kept, re-pointed; more wells to make it
easier); three new places — the DOOR HQ Rooftop ("not what you'd
expect"), Tartaria, the Bermuda Triangle ("it's a right triangle"); and,
super optional after skateboarding, a water polo mini game in the
natatorium built from the user's water polo repository. HQ plan 9.3
...
### 2026-09-16 rev 63 — THE THIRD RING + 7.7 wave 2 begins: 345 · THE BERMUDA TRIANGLE (local)
The hall's third level: THE GALLERY, a ring slab 3.3 m over the mezzanine
(`shell.ring3`), one curved flight off the mezzanine's walkway, the four
EXPLORATION doors on it (Arcane Engineering, IT, the Observatorium, the
Executive Suite — the Bureau's gate untouched inside) and the mezzanine
left to the bays and the elevator; "nothing up here launches, everything
up here looks" (THE GALLERY WATCH, Claude's draft, A15). And the first
wave-2 site: Room 345 · THE BERMUDA TRIANGLE (A10 Hollow) — "it's a right
triangle": two shoals meeting at a deep hypotenuse, the sandbar the width
of two men the way over, the corner buoys, the lighthouse the compass
points at, five aircraft filed as a crossing in 1945; THE WEIR — Room 8's
