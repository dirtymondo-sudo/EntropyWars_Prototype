...
test('THE SOURCE SITES: the XP hold in a party fight, the pool at the commit, the experience card on the debrief', () => {
    ...
    assert.ok(BT.indexOf("_xe.innerHTML = _vicBuildExperience(_vicXpParty)") < BT.indexOf("_vicLayoutSync({ tab: 'rewards' });", BT.indexOf('function _showResultOverlayBody')) && BT.indexOf("_vicPlayExperience(_vicXpParty)") > BT.indexOf("resultOverlay.classList.remove('hidden');", BT.indexOf('function _showResultOverlayBody')), 'built before the layout sync, played after the overlay shows');
