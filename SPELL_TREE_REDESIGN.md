...
**Status: PHASE A SHIPPED 2026-08-07** (branch `claude/spell-tree-redesign-y6z5h1`, token `20260807h-cors`).
Live: tree engine in data.js (`CLASS_TREE` derived from the ring-ordered
`CLASS_SPELL_LEARN_ORDER`, curated `RACE_TREE` for 79 races + first-4 fallback,
`isTreeLoadoutLegal` / `treeLegalSubset` / `buildTreeLegalLoadout`), the
party-builder `SpellTreePanel` (SVG paths, reachable-glow, click-to-auto-path,
sever-blocking unequip, clash "sealed" pass-throughs), tree-legal AI/random/
optimize/auto-fill loadouts, host-side online validation (receipt + createUnit),
job-tree tier retags (ring = tier), §2.1 explicit single-status fixes
