...
### Current priority — move to Phase 6 (user direction, 2026-09-11)
...
**Implemented and locally validated; Phase 6 and Chivalry AI scoring remain open.** Refreshed latest main `99ff7c86f82113cc46fa2aee6f3c43ca16cd0356`. Downloaded and Git-blob-verified changed battle.js, data.js, online.js, index.html, CLAUDE.md, PLAYTEST_NOTES.md and recovery-protocol.test.js, plus the new net-snapshot-cycle.test.js. Other relevant files matched the pinned tree. Preserved the current online cycle fix (`_lastDamageSourceId` and safe snapshot serialization), the previous Trick Room reset, and all completed AI changes. Main remained unchanged at final verification.

**AI-04b-L2 — Medium, fixed locally: Chivalry can land inside obstacles or retain an invalid elevation.** The actual interception loop checked only bounds and column occupancy, then assigned x/y without z. A ward beside a wall, blocking object, tower, deployed obstacle or turret could pull the knight into a blocked tile. A different surface height retained the knight's old z. The loop now resolves the nearest standing surface to the ward's elevation and checks the existing `canOccupy3D` predicate before assigning all three coordinates. That predicate covers surfaces, terrain, same-surface occupants, objects, towers, deployed blockers and turrets. A unit on another elevation follows the shared 3D occupancy contract. Candidate direction order is unchanged. When every adjacent landing is blocked, interception still consumes the pledge and transfers the hit from the guardian's current position, preserving the existing no-open-tile fallback. This is a landing check, not a pathfinding/range restriction on the oath.

**Validation:** 19 new production-boundary checks pass; 12 fail on the pinned original battle.js, seven controls pass before/after. The tests execute the actual recursive interception and early immunity pipeline with actual nearestWalkableZ/canOccupy3D/unitAt3D functions and controlled board queries. They check both player seats, blockers, missing surfaces, elevation, board edges, no-open-tile fallback, one-hit consumption, raw hit/options forwarding, excluded triggers and guardian immunity. They stop before downstream damage arithmetic/rendering and do not establish final damage, hazards, death callbacks, full doSpell legality or live guest reception. Full package test command through bundled Node (`node --test *.test.js`): **794 total, 792 passed, zero failed, two existing skips**. Syntax: **110/110 JS files**, plus **12 inline entry scripts and valid import-map JSON**. No browser playtest, AI simulation, performance capture or deployment.

**Online and scope:** no new fields, references or relays. The host remains authoritative; existing unit position/pledge state synchronization carries the changed landing. The latest online snapshot regression is included in the full suite. Live host/guest animation and positional acceptance remain unverified. No AI scorer, picker, damage formula, weights or stamp change in this batch.

**Further contract observation:** interception precedes the ward's protection, realm and elemental immunity checks, while those checks run on the redirected guardian. Thus a scorer cannot assume a protected ward prevents pledge consumption, or treat guardian loss as free shield value. This ordering is source-traced and unchanged; whether to alter it is a separate engine contract question. Landing hazards/displacement presentation are also unchanged and remain runtime evidence gaps.

**Delivery:** `ENTROPY_WARS_PHASE6_CHIVALRY_LANDING.zip`: complete battle.js → R2; index.html → Render (fresh shared token `20260912-chivalry-landing-01-cors`); chivalry-landing.test.js, AI_REDESIGN.md, CLAUDE.md and this plan → repository. Evidence includes pinned tree, before/after/full/syntax logs and hashes. Sync runtime files to the repository as well. No commit, push or deployment.

**Exact next task:** implement AI-04b Chivalry legal-target and net-interception scoring, using this corrected landing position and guardian-specific damage/kill risk; test pledge replacement and ordinary/Simul execution. Keep the interception-before-ward-immunity distinction explicit. Then resolve Trick Room Simul/effective-SPD ordering, coupled Tune/Pulse valuation and AI-06. VFX cleanup remains deferred. This prerequisite landing fix does not close utility AI scoring or Phase 6.

### Latest continuation — 2026-09-11: Phase 6 / AI-04b utility contracts and Trick Room reset

**Implemented one prerequisite fix; Phase 6 remains in progress.** Refreshed main `844c4ba424963f301a0dd24e952f2ce08cc5838a` contains the wide-beam delivery. Verified the Git blob hashes of ai.js, battle.js, state.js, data.js, AGENTS.md, CLAUDE.md, AI_REDESIGN.md, index.html and this plan before editing. Main remained unchanged at final verification. VFX work stays deferred.

**AI-04b-L1 — Medium, fixed locally: Trick Room crosses match boundaries.** `state` is a persistent object. The cast assigns `_trickRoomRounds`; `buildBlitzTurnOrder` consumes it, but the four existing match/roster reset blocks did not clear it. Ending a battle with rounds remaining and preparing another battle could therefore reverse its first order without a new cast. Added an initial zero in state.js and zeroing alongside the other match effects in `randomizeParty`, `prepareBattleStateFromCurrentBuilds`, `resetGame` and `applyPartyBuild` (battle.js). Ordinary in-match duration, speed ordering and casting behavior are unchanged. The host serializer already includes this scalar; it now transmits the cleared value as well. No new relay or AI weights. This does not establish every match-transition route by browser reproduction.

**AI-04b-G — Medium, open: Chivalry needs transfer valuation, not shield value.** Production AI probes with an available living ally return null target and zero candidates. Engine contract: ally-only, no self, a persistent one-hit pledge; a new pledge clears the knight's prior ward if it still belongs to that knight. `applyDamageToUnit` intercepts a positive hit only with an enemy sourceUnit, consumes the pledge, moves the guardian beside the ward and recursively applies the same raw damage/options with `_chivalryRedirected`. Damage mitigation then applies to the guardian. It neither creates HP nor promises interception of source-less damage. A scorer must compare the ward's prevented loss/kill risk against guardian loss/kill risk, legal range/LOS/elevation, current pledges and displaced position. Reusing `v.threatFn` for an ally would also be wrong: that field is built for the deciding unit's defenses. Guard's adjacent landing loop checks bounds/occupancy but not terrain or height; source-review residual for an engine landing-policy pass, not fixed or behavior-tested here. Chivalry (`kind: guard`, a spell) is distinct from Simul's separate basic `type: guard` order.

**AI-04b-T — Medium, open: Trick Room lacks an AI picker/scorer and Simul does not apply reversal.** Production AI probes return null target and zero candidates. Blitz applies the effect on the next order build, reads effective SPD, decrements once per build and leaves the already-built order intact; equal-speed Quickdraw/tie rules are retained. Simul `_resolvePlans` instead sorts by plan priority, raw `unit.spd`, then initiative. Controlled execution of that actual comparator returns the faster player first with both zero and three Trick Room rounds. Simul still reaches the common Blitz end-of-round sequence through `_finishSimulTurn`, so the duration can count down while its ordering benefit is absent. Before adding a positive Simul score, reconcile effective-speed/reversal behavior with the mode's priority and already-locked plan contract. Blitz valuation should compare future useful ordering gains/losses, exclude equal-speed gains, account for current remaining duration (recast refreshes; it does not toggle), and preserve fog visibility. No arbitrary utility floor was added.

**AI-04c-V — Medium, open: Tune and Pulse share a frequency valuation gap.** Tune cycles the player's entire live lattice through Infrared → Ultraviolet → Gamma → Infrared. The engine permits Tune with one live prism; the AI returns zero below two and a constant 12 otherwise, independent of frequency, topology, targets or effects. A one-prism network may reasonably have no immediate benefit, but this is not an engine legality restriction. At current default MP value 0.529 and canonical 75 MP cost, net score is `12 - 75 * 0.529 = -27.675`; supported lower trained weights can admit it. Do not infer universal unreachability or fix it with an MP exemption.

Pulse valuation is also required for a meaningful Tune comparison. The production pulse scorer returns 140 for the same one-enemy beam in all three frequencies. Controlled `doPulseLattice` calls at zero spell power emit raw damage 109/95/85 with tech+burn / alien+DEF stage -1 / anomaly+slow respectively. These are pre-mitigation call arguments, not actual damage outcomes. Frequency also affects crossing and end-of-round beam hits. Value the *next* frequency relative to the current one using real geometry, player-wide ownership, target susceptibility, status headroom, plausible exposure and resource/action opportunity cost. Do not assume Tune can choose any desired frequency in one cast. Prior self-cast coordinates and wide-beam fixes remain intact.

**Validation:** `trick-room-lifecycle.test.js`: 12 passed; five fail on untouched baseline and seven controls pass on both. Tests execute the four production data-reset blocks, actual Blitz order builder and actual host serializer with controlled dependencies. Covers repeated resets, fresh-match order, three future reversed builds, live SPD, Quickdraw ties, already-built order and active/cleared serialization. It does not execute the entire renderer/account/map/UI reset functions or guest receive path. Full package test command via bundled Node (`node --test *.test.js`; npm is not used): **772 total, 770 passed, zero failed, two existing skips**. Syntax: **108/108 clean**; the diagnostic was syntax-checked again after correcting its extraction boundary and executed successfully. `check-ai-utility-contracts.js` emits reproducible current-source observations; its output is not a passing gameplay acceptance test. No browser playtest, simulation, performance capture or deployment.

**Delivery:** `ENTROPY_WARS_PHASE6_TRICK_ROOM_RESET.zip` contains complete battle.js and state.js → R2; index.html → Render (fresh shared token `20260912-trick-room-reset-01-cors`); lifecycle test, diagnostic, AI_REDESIGN.md, CLAUDE.md and this plan → repository only. Evidence contains before/after/full/syntax logs, observations and pinned tree metadata. Runtime files should also be synced to the repository. No commit, push or deployment.

**Exact next task:** continue AI-04b with Chivalry legal-target and net-interception scoring, including guardian mitigation and pledge replacement tests. Resolve Trick Room's Simul/effective-SPD contract before implementing its mode-aware scorer; then implement coupled Tune/Pulse frequency valuation and continue AI-06 endgame evaluation. AI-05 destructive-breach and null-Simul-re-aim residuals remain open. This pass advances specific contracts and fixes a prerequisite lifecycle defect; it does not close utility scoring or Phase 6.

### Latest continuation — 2026-09-11: Phase 6 / AI-05 wide-beam direction and footprint

**Implemented and locally validated; not deployed.** Pinned main `3a00af7735cb8e96681bc1c771841794c85cb368` contains the previous prism delivery. Git blob hashes match local ai.js, battle.js, data.js, index.html, AGENTS.md and the three history/review documents before edits. Main remained at that revision after validation.

**AI-05 wide-beam residual — Medium, fixed locally:** Plasma Cannon and Tsunami now count passable side lanes after the center ray determines reach. The shared AI footprint mirrors the engine's width-2 right lane and width-3 two-sided lanes, including diagonal offsets, board clipping and unique cells. Side cells do not impose independent LOS or extend reach past a blocked spine. Target discovery chooses total per-direction spell value, retaining an aligned aim coordinate and a victim identity for the existing Simul re-aim contract. A side-lane enemy's off-axis coordinates are no longer mistaken for the firing direction.

Scoring, discovery, hypothetical movement and cast-time re-aim use the same footprint. Movement sums the direction's victims and charges MP once; it evaluates from a copied caster at the destination height. Ordinary and Simul re-aim retain current visibility/concealment gates and use value rather than victim count, with preferred identity as a tie-break. These shared line/linePush paths also improve narrow-beam direction valuation; numerical weights are unchanged. No new network fields, guest execution, engine effects or relay changes. AI stamp: `v4.5-2026-09-12-wide-beams`.

**Validation:** 28 new checks pass; 26 fail before and two controls pass on both versions. Canonical spell data comes from load-data.js. Tests execute the production AI closure, engine directional footprint function, Simul candidate conversion and Simul spell executor with controlled board/resource/damage and doSpell boundaries. Covers all eight directions for both wide spells, side-only hits, full-direction value, wall/LOS distinctions, clipping, range, hypothetical elevation, one MP charge, moved victims, ordinary completion/retry, both player seats, hidden/protected/dead exclusions, narrow-beam control and resource gates. Full package script through bundled Node (`node --test *.test.js`; npm unavailable): **760 total, 758 passed, zero failed, two existing skips**. Syntax **106/106 clean**.

**Limits and remaining evidence:** no browser playtest, AI simulation, real damage/displacement run, performance capture or live host/guest acceptance. Tests compare geometry with the engine's non-boring footprint; prediction of destructive wall boring stays open. Cast-time scoring recomputes current victims without rebuilding the full tactical vision/focus context. Existing Simul behavior when re-aim returns null still retains the old aim and may whiff; this batch validates successful re-aim, not cancellation of that existing fallback. Keep these limits separate from the fixed side-lane discovery/direction defect.

**Delivery:** `ENTROPY_WARS_PHASE6_WIDE_BEAM_FIXES.zip` contains complete ai.js → R2; index.html → Render, shared token `20260912-wide-beams-01-cors`; ai-wide-beam.test.js, AI_REDESIGN.md, CLAUDE.md and this plan → repository only. Runtime files should also be synced to the repository. Includes validation logs, pinned source-tree metadata and file hashes. No commit, push or deployment.

**Exact next task:** continue AI-04b guard/Trick Room target/scorer correspondence and frequency-aware Tune valuation, then AI-06 mode-aware endgame evaluation. Track null-re-aim Simul behavior and destructive-breach prediction as AI-05 residual evidence/work; Phase 6 remains open. VFX callback work stays deferred.

### Latest continuation — 2026-09-11: Phase 6 / AI-04c prism self-cast coordinates

**Implemented and locally validated; not deployed.** Current main is `043dede2ada453c7ccc9d022438988107eeba64a`, which includes the preceding spell-routing delivery. Verified Git blob identities against pinned GitHub files for ai.js, battle.js, data.js, index.html, AGENTS.md and the three review/history documents; main remained unchanged after validation. Continued the existing source snapshot with those identities verified.

**AI-04c — Medium, fixed locally:** Pulse Lattice and Tune Frequency now return the caster from `findSpellTarget`, supplying x/y/z and identity to ordinary execution and Simul candidate conversion. Removed their obsolete no-target exemptions. A positive Pulse candidate previously reached the ordinary executor with null target and threw before `doSpell`; six new regression checks fail against the unchanged AI. Existing engine calls, resource gates, failed-action memoization and completion delays remain intact. AI stamp: `v4.4-2026-09-12-prism-selfcast`.

**Ownership suspicion resolved without a code change:** placement uses `ownerUnitId` for per-caster limits, while `computeMirrorNetwork`, `_mirrorSpellBlockReason`, Tune and Pulse deliberately aggregate live mirrors by `owner === unit.player`. Teammates can contribute to the same player's lattice. Replacing that filter with caster identity would be a regression. Tests use the actual engine network function and cover both player seats, teammate-owned mirrors, opposing/dead mirrors, disconnected geometry, beam versus elevated-volume hits and unseen enemies.

**Tune reachability qualification:** canonical data after normalization sets Tune's cost to 75 MP. Its existing value of 12 loses to the default MP valuation (0.529 per MP); a supported trained weight of 0.1 admits it and exposes the latent target failure. The tests explicitly cover both states. This batch preserves its value and two-prism tactical threshold; engine legality allows one prism. Frequency-aware tactical valuation remains open, not claimed solved.

**Validation:** 12 new production-closure/network tests pass; six fail before and six controls pass before/after. Full package test script executed directly with bundled Node (`node --test *.test.js`, because npm is unavailable): **732 total, 730 passed, zero failed, two existing skips**. Syntax: **105/105 clean**. Tests double `doSpell` and board/engine boundaries; they do not validate real damage, frequency effects, browser gameplay or host/guest acceptance. Simul `_candToStep` and alternate beat execution were source-reviewed: the concrete self target preserves coordinates instead of Simul's null fallback to 0,0. No new runtime fields or relays; host authority and existing spell effects remain unchanged.

**Delivery:** `ENTROPY_WARS_PHASE6_PRISM_FIXES.zip`: complete ai.js → R2; complete index.html → Render, shared token `20260912-prism-selfcast-01-cors`; ai-prism-selfcast.test.js, AI_REDESIGN.md, CLAUDE.md and this plan → repository only. Sync runtime files to the repository too. Evidence includes before/after, full-suite, syntax logs and hashes. No commit, push, deployment, browser playtest or AI simulation.

**Exact next task:** implement AI-05 wide-beam direction/footprint correspondence for Plasma Cannon and Tsunami across scoring, target discovery, hypothetical moves and cast-time/Simul re-aiming. Then AI-04b guard/Trick Room and frequency-aware Tune valuation, followed by AI-06 mode-aware endgame evaluation. Phase 6 remains open; VFX callback work stays deferred.

### Latest continuation — 2026-09-11: Phase 6 / AI-04 spell dispatch

**Phase 6 is in progress. Implemented and locally validated; not deployed or browser-playtested.** Refreshed the complete repository snapshot at main `82906801a763c0ce5fe943bdde48980d60ee7776`; main remained at that revision after validation. The earlier navigation/beam-spine fixes and latest Ego Death/Time Rewind delivery are preserved.

**AI-04a — Medium, fixed locally: unreachable Hit a Lick and Purify handlers.** Both `steal` and `cleanseArea` scorer and target-picker branches were nested inside `kind === 'utility'`, making them unreachable for their actual kinds. Normal `scoreSpells` consequently discarded both spells for lack of a target. Moved the existing handlers into direct kind dispatch and made missing targets score zero. Existing values, weight routing, resource gates, execution delays, failure memos, engine mechanics and online relays are retained. AI stamp: `v4.3-2026-09-12-spell-routing`.

**Validation:** 13 new production-closure checks pass; 10 fail against the unchanged pinned AI and three controls pass before/after. Covers Key/item targeting and valuation, allied debuff removal, enemy buff removal, empty/null targets, visibility/protection/range/LOS exclusions, AP/MP/affordability/silence, candidate-to-executor coordinates, completion delays and rejected-action memoization. Uses canonical spell/status data through `load-data.js`, controlled damage/threat values, board queries and a doubled `doSpell`; does not claim real engine damage/removal or live acceptance. The old champion source guard was strengthened to require direct dispatch and explicit null-target guards; its previous expression-presence check passed on the broken baseline. Full package test command: **720 total, 718 passed, zero failed, two existing skips**. Syntax validation is recorded in the package evidence. No browser playtest, AI simulation, real-GPU or live multiplayer test.

**Inventory:** canonical `SPELL_BY_ID` has **501 spells / 65 kinds**. `check-ai-spell-dispatch.js` records direct scorer/target-picker presence per kind, with spell IDs and explicit limitations. It inventories source dispatch, not complete tactical or engine coverage. Before this fix, four kinds lacked reachable direct scorers (`steal`, `cleanseArea`, `guard`, `trickRoom`); after, `guard` and `trickRoom` remain. The generic positive floor is unchanged: their normal candidates are rejected by missing target discovery before reaching it. Direct calls with unsupported kinds can still reach the floor, and guarded known-kind branches can fall through with null targets; do not describe that as demonstrated wasteful casting by the normal planner.

**Remaining concrete traces:**

- **AI-04b — Medium, open:** Chivalry (`raceChivalry`, `guard`) and Trick Room (`raceTrickRoom`, `trickRoom`) have engine branches but no direct AI target picker/scorer. The ordinary spell candidate loop skips them. Define actual guard interception value/ally eligibility and mode/initiative-aware reversal value before adding them; do not blanket-zero utility or invent a positive floor.
- **AI-04c — Medium, open:** `pulseLattice` and `tuneFrequency` have scorers but no target picker. They are allowed through `noTargetKinds`, while the generic spell executor dereferences `action.target.x/y/z`. The source therefore contains a null-target execution path when a positive candidate is admitted. Also inspect mirror ownership: the scorer reads `m.owner`, whereas placement counts elsewhere use `ownerUnitId`. This is a source-confirmed contract gap, not a live reproduced crash; actual mirror-state reachability/value must be covered in the next regression batch.
- **AI-05 wide-beam residual — Medium, open:** only `racePlasmaCannon` (width 2, range 4) and `raceTsunami` (width 3, range 4) currently use wide `line`/`linePush` footprints. `battle.js:getLineSpellLaneOffsets` and `_applyLineDamage` add passable perpendicular lanes after the spine determines reach. AI `_lineRayTilesAI`, `scoreSpell`, target discovery and hypothetical move scoring still inspect the spine only. A side-lane hit cannot simply return that enemy as the aim: execution uses the sign of the aim vector and re-aims at cast time. The follow-up must preserve firing direction with an aligned aim tile, account for complete per-direction value and unique victims, and cover cast-time/Simul re-aiming. Destructive breach prediction remains a separate boundary.

**Delivery:** `ENTROPY_WARS_PHASE6_AI_ROUTING_FIXES.zip` contains complete files. `ai.js` → R2; `index.html` → Render with shared token `20260912-ai-spell-routing-01-cors`; both tests, dispatch inventory tool, AI_REDESIGN.md, CLAUDE.md and this plan → repository only. Sync runtime files to the repository too. Before/after dispatch JSON, test logs, hashes and baseline identity are included as evidence. No commit, push or deployment.

**Exact next task:** continue Phase 6 with AI-04c prism self-cast target/executor and mirror-ownership correspondence, then implement AI-05 wide-beam direction/footprint correspondence for Plasma Cannon and Tsunami. Follow with AI-04b guard/Trick Room, then AI-06 mode-aware endgame evaluation. Keep VFX callback cleanup deferred as directed. Phase 6 and AI-04 remain open.
...
Neon Grid, Fractal Tunnel, Kaleidoscope, Spectrum Burst, Prism Refraction and Stat Rings now return `_sigRunOwned`, disposing new allocations on cap refusal or scene loss between construction and registration. All six retain entry/null contracts and normal animation. Spectrum's immediate pool/flash/shake/bloom effects are preserved; Prism's scheduled animation still owns its split flash and one-time arrival/impact. No new relays or authority changes: both viewers use the same helpers.

The audit also found a shared-material disposal defect in the production wireframe builder: bars and joints alias one instance material. `_sigDisposeGroup` now deduplicates materials by identity within each cleanup and includes `userData.wireMat`, which owns the material even if edge extraction throws, returns no position attribute, or yields no edges. All current wireframe callers create per-effect materials. Cached textures, `_ew_shared` geometry and engine Sprite geometry retain their existing policy. No ownership is transferred across effects; no global disposed-material cache is introduced.

**Validation:** 51 new production-builder tests pass; 27 fail on the pre-six local baseline and 24 controls pass before/after. The same **51/51 pass using real Three.js r128 objects** with instrumented resource disposal; the real-object pre-fix run also reproduces 27 failures. Default tests use dependency-free doubles; optional `EW_THREE_MODULE` loads a supplied engine module. Both execute the production wireframe builder and the real Spectrum Burst nested by Prism Refraction. Tests cover cap/scene refusal, no-scene early exit, normal completion, retirement/stale callbacks, tick exceptions, wireframe errors/empty output, material alias arrays, animation parameters, spectrum colors, indexed tube growth, one-time impact, nested retirement and both Stat Ring directions. These are headless object tests, not WebGL/shader rendering or live host/guest acceptance. Prism mapped impact is a recorded stub; downstream mapped effects are not newly audited here.

Full package test command via bundled Node: **693 total, 691 passed, zero failed, two existing skips**. Syntax: **101/101 clean**. This cumulative package has 73 focused checks across its two new test files. No asset binaries or embedded asset URLs changed.

**Delivery:** `ENTROPY_WARS_SIX_HELPER_FIXES.zip` supersedes `ENTROPY_WARS_CRESCENT_ORB_FIXES.zip`. Complete `three-vfx-effects.js` → R2; complete `index.html` → Render with fresh shared token `20260912-six-helper-01-cors`; both tests and updated plan/CLAUDE.md/inventory → repository only. Sync runtime files to the repository too. Evidence and hashes included. No commit, push or deployment.

**Exact next task:** zero `return _sigRun(` sites remain, but this is not full transitive lifetime coverage. Review downstream Psychosis ownership and subsequent Ego Death emission timers, then VFX-04 endpoint/list visibility. Audit retained `_sigRun` callers that inspect returned entries only if their cleanup remains unproven. LIFE-06/VFX-03 remain partial; application cache warmup and browser/online acceptance stay separate.

### Latest continuation — 2026-09-11: LIFE-06 / VFX-03 Crescent Slash and Orb Burst

**Implemented and locally validated; not deployed or browser-playtested.** Downloaded complete main archive `4f9e0fdcbd1fffa970b62a1127be7739f7e85169`. Its runtime files and CLAUDE.md match the prior magic-ownership delivery, confirming those changes are now in the repository (live deployment remains unverified).

Crescent Slash and Orb Burst now return `_sigRunOwned`. Crescent Slash releases both ribbon geometries, its three materials and the private scrolling smear texture when registration is refused. Its main material already owns that texture's disposal; the echo shares the texture without owning it. Orb Burst retains its preallocation cap guard and now releases instance materials if the scene disappears between the initial scene check and runner registration. Its sphere/plane geometries are application caches marked `_ew_shared`; those and engine Sprite geometry remain retained. No generic disposer, animation, particle emission, asset URL or relay changes. Host and guest use the same local helpers and retain existing payloads.

**Validation:** `crescent-orb-ownership.test.js`: 22 passed. Four failures reproduce on the untouched pinned source: Crescent Slash cap and scene-loss refusal, plus both Orb Burst modes on scene loss. Eighteen controls pass before and after. Tests run production builders, ribbon generation, animation, runner and disposal against controlled Three.js doubles. Checks cover private texture disposal, shared geometry retention across casts, missing scene, early cap guard, natural completion, explicit retirement, stale callbacks, tick exceptions, slash sweep/echo/scroll/flecks, orb in/out shell timing/radius/flash and optional branches. No GPU, browser or live multiplayer validation is claimed.

Full package test command via bundled Node (`node --test --test-reporter=tap *.test.js`): **642 total, 640 passed, zero failures, two existing skips**. Syntax: **100/100 clean**. Complete snapshot includes assets; asset binaries are unchanged.

**Delivery:** `ENTROPY_WARS_CRESCENT_ORB_FIXES.zip`: complete `three-vfx-effects.js` → R2, complete `index.html` → Render (fresh shared token `20260912-crescent-orb-01-cors`), new test and updated review plan/CLAUDE.md/inventory → repository only. Sync runtime files to the repository too. Evidence and baseline hashes are included. No commit, push or deployment.

**Exact next task:** audit Neon Grid, Fractal Tunnel, Kaleidoscope, Spectrum Burst, Prism Refraction and Stat Rings, the six remaining return-only `_sigRun` consumers. Inspect actual resource ownership and normal animations before changing calls. Then downstream Psychosis ownership and Ego Death direct emissions, followed by VFX-04 endpoint/list visibility. LIFE-06/VFX-03 remain partial. Application cache warmup and browser/online acceptance remain separate.
...
**Implemented and locally validated; not deployed or browser-playtested.** Continued directly from this session's whiteout/seal/gas batch, based on the five verified GitHub main blobs recorded below. This is a cumulative seven-fix delivery; it includes the preceding four fixes and preserves the main cannon/emission changes.

`_sigAuroraCurtain3D` and `_sigSpiralBeam3D` now return `_sigRunOwned`, preserving their entry/null contract while disposing fresh instance geometry/materials on active-cap refusal or scene loss between construction and registration. Shared aurora textures stay cached. Existing normal animation, ribbon vertices/colors, beam endpoint placement, extension, particles and options are unchanged. `_sigBadTrip3D` now schedules its two delayed skulls with `_fxDelay`. The immediate skull stays immediate; later skulls are canceled on retirement and reject queued callbacks after a fresh cast, including after the 150 ms skull has appeared. Existing category/suppression gates remain. Both players use these local helpers; no relay or authority changes.

**Validation:** 16 new production-helper tests pass; six fail on the preceding whiteout/seal/gas baseline and ten normal-behavior controls pass before/after. Tests execute real lifetime/animation functions with controlled rendering objects; Bad Trip's downstream visual primitives are recorded doubles, so their geometry/DOM ownership is not established by this batch. Coverage includes cap/scene-loss allocation cleanup, missing scene, custom options, finite ribbon vertices and colors, beam position/extension and particle payloads, normal completion, early retirement, stale frames, skull timing/options and partial emission. All instance resources in the two audited geometry helpers dispose exactly once; cached textures are not disposed.

Full package test command via bundled Node (`node --test --test-reporter=tap *.test.js`; npm unavailable): **576 total, 574 passed, zero failures, two existing skips** (fitted-outfit rigs and server boot dependencies). Syntax: **94/94 clean**. Across both batches: **40 new passing tests, 17 demonstrated baseline failures**. These are local controlled tests, not WebGL/live host-guest acceptance or full-main CI.

**Delivery:** `ENTROPY_WARS_CONTINUED_VFX_FIXES.zip` supersedes this session's whiteout/seal/gas zip. Complete three-vfx-effects.js → R2; index.html → Render with shared token `20260910-aurora-spiral-trip-retirement-01-cors`; both tests, updated docs and evidence → repository only. No asset binaries changed. No commit, push or deployment.

**Exact next task:** audit `_sigMagicCircle3D`, `_sigMagicOrb3D` and `_sigLightPillar3D` returned-entry refusal cleanup first (especially the shared circle used by Spear Prison and Gas Cloud), with real-helper allocation and normal-animation regressions. Then the remaining return-only consumers: Crescent Slash, Orb Burst, Neon Grid, Fractal Tunnel, Kaleidoscope, Spectrum Burst, Prism Refraction and Stat Rings. Do not infer transitive cleanup from the caller tests. Continue `_sigPsychosis3D` and subsequent bespoke timer ownership, then VFX-04 endpoint/list visibility. LIFE-06/VFX-03 remain partial; application cache warmup and browser/online acceptance remain separate. This entry supersedes older continuation ordering.

### Latest continuation — 2026-09-10: LIFE-06 / VFX-03 whiteout, seal and gas batch

**Implemented and locally validated; not deployed or browser-playtested.** Refreshed the current GitHub main files and verified all five edited baseline files by Git blob hash (recorded in BASELINE_HASHES.txt). The older review-work directory was stale; this delivery continues adversarial-current and preserves the cannon/emission fixes already in main.

Four fixes: `_sigWhiteout3D` second shock ring, `_sigSpearPrison3D` finisher and `_sigGasCloud3D` staggered puff creation now use `_fxDelay`. These three sites create effects and own no disposal. Retirement cancels pending work and rejects queued callbacks after a new cast, including a prison finisher that has already queued Whiteout's second ring and gas clouds that have partially emitted. `_sigRuneSphere3D` now returns `_sigRunOwned`, preserving its entry/null contract while disposing its six instance geometry/material resources when registration is refused at the cap or after scene loss. Shared glyph textures stay cached. Both online viewers execute these same local helpers; relay payloads and authority are unchanged.

**Validation:** 24 production-helper checks pass; 11 reproduce failures on unchanged source and 13 normal/suppression/cleanup controls pass before and after. Tests exercise real lifetime, animation and spear-building functions with rendering doubles. Coverage includes timing, custom colors/options, finisher disabled, gentle gas, missing scene, partial emission, nested emission, new lifetime, cap refusal, normal completion and repeated retirement. Rune/gas instance resources dispose exactly once; the existing spear shared-material traversal policy is unchanged. Full package test command via bundled Node (`node --test --test-reporter=tap *.test.js`; npm unavailable): **560 total, 558 passed, zero failed, two existing skips** (fitted-outfit rigs and server boot dependencies). Syntax: **93/93 clean**. This is accumulated local-workspace validation, not full-main CI or WebGL/live host-guest acceptance.

**Delivery:** `ENTROPY_WARS_WHITEOUT_SEAL_GAS_FIXES.zip`; complete three-vfx-effects.js → R2; index.html → Render with shared token `20260910-whiteout-seal-gas-retirement-01-cors`; test, documentation and evidence → repository only. No asset binaries changed. No commit, push or deployment.

**Exact next task:** audit `_sigAuroraCurtain3D` and `_sigSpiralBeam3D` returned-entry refusal paths, then the other return-only `_sigRun` consumers. The earlier sweep's claim that all 28 remaining callers self-cleaned was too broad: merely returning a refused entry does not dispose the built group, as Rune Sphere demonstrates. Both named helpers still return `_sigRun` without local refusal cleanup. Add allocation/normal-animation regressions before changing them. Continue `_sigBadTrip3D` staggered skull emissions and subsequent bespoke callbacks, then VFX-04 endpoint/list visibility. LIFE-06/VFX-03 remain partial; application cache warmup and browser/online acceptance remain separate. This entry supersedes older continuation ordering.
...
**Validation:** 15 production-helper tests pass; six cancellation checks fail against unchanged source and nine controls pass. Tests execute actual lifetime and animation owners with rendering doubles; sword construction and downstream visual helpers are doubled. Full package test command via bundled Node 24 (`node --test --test-reporter=tap *.test.js`; npm unavailable): **509 total, 507 passed, zero failed, two skipped**. Syntax: **91/91 clean**. Existing fitted-outfit rig and server boot dependency skips remain. No WebGL or live host/guest acceptance.

**Delivery:** `ENTROPY_WARS_SLASH_JAWS_FIXES.zip` contains complete files. `three-vfx-effects.js` → R2; `index.html` → Render (shared token `20260910-slash-jaws-retirement-01-cors`); test, logs, inventory, CLAUDE.md and plan → repository only. No commit, push or deployment.

**Exact next task:** audit `_sigCannonShot3D` detached cannonball cleanup (line 12624): this timer DISPOSES resources and must not simply be canceled. Bind disposal to retirement while preserving normal completion, then continue remaining bespoke emissions and VFX-04 endpoint/list visibility. LIFE-06/VFX-03 remain partial; application-owned weapon-cache warmup and browser/online acceptance remain separate.
...
**Implemented and locally validated; not deployed or browser-playtested.** The transitive `_fireWall` audit is closed: its recipe emissions were already `_fxDelay`-owned, `_spawnEffect` owns the delayed recipe layers, and of the three wall-mapped spells (`racePlaguefield`, `rampart`, `raceWalkThePlank`) only Walk the Plank reaches `_spell3DGeometry` — `_sigPlank3D`, which has no delayed emissions but leaked two box geometries and two materials whenever `_sigRun` refused (active cap, or the scene lost between its early check and registration).

**The sweep:** a source sweep of all 76 `_sigRun` call sites found 47 helpers that never read the returned entry and therefore could not clean up a refusal (candles, bones, cross, GLB fist, skull crack, tarot ×3, sleigh, missile drop, stand sword/fist, slash combo, shield bash/ring, jaws, claws, cannon, bow, arrow rain, cauldron, Excalibur, iai, blade waltz, sword wave, spotlight, crown, parry, plank, UFO, UFO fleet, sacred rings, skull, bone rain, flesh mound, crystal ball, tome, wood cross, music notes, spear prison, gas cloud, naughty list, present, both Calcify rigs, and the descent jet / mapped-AoE missile model groups). The disposal rule inside `_sigRun`'s `finish` is now the one function `_sigDisposeGroup` (skip `_ew_shared` GLB geometry, dispose every material including arrays, never textures), and a new `_sigRunOwned(group, ms, tick)` calls `_sigRun` and applies that rule to the group when registration is refused. All 47 sites call `_sigRunOwned`; the 28 callers that own their refusal cleanup (they check the entry) still call `_sigRun`, so nothing double-disposes. The `_sigRun` contract itself is unchanged. Both online viewers run these local helpers; relay payloads and authority are unchanged.

**Asynchronous asset attachment (inspected, no change):** the file has two async loaders. `_wpnLoad` (GLTFLoader) writes only into `_wpnCache` and marks geometry `_ew_shared`; casts clone the cached root and fall back procedurally while it streams, so no callback ever attaches into a live effect group. `_loadCachedTex` (TextureLoader) fills a cached texture object in place. The boot warmup (`window.setTimeout` at the weapon library, plus the staggered `_wpnLoad` drip) is application-lifetime and must stay outside `_fxCancelDelays`. Nothing to retire.

**Validation:** new `sig-refusal-cleanup.test.js` — 7 tests pass; 5 fail against the unchanged baseline (source scan, refused-group disposal with shared geometry / material arrays / textures, exactly-once finish, plank cap and vanish refusal) and 2 controls (plank animation, missing scene) pass before and after. `descent-lifetime.test.js`'s harness gained a `_sigRunOwned` double beside its `_sigRun` double (its production slice now calls the new name). Full `npm test`: **464 total, 462 passed, 0 failed, 2 skips** — the Phase 6 starter and lunar-lander assertions recorded as failing in earlier entries pass on this synced checkout. Syntax: **88/88 clean**. Rendering objects are controlled doubles; no WebGL, browser or host/guest acceptance.

**Source and delivery:** edited in place on the synced repo checkout (`1c9af7e`). three-vfx-effects.js → R2; index.html → Render (token `20260911-sig-refusal-cleanup-01-cors`); sig-refusal-cleanup.test.js, descent-lifetime.test.js, inventory, CLAUDE.md and plan → repository only. Complete files in `ENTROPY_WARS_SIG_REFUSAL_FIXES.zip`. No commit, push or deployment.

**Exact next task:** the remaining direct `window.setTimeout` emission sites in the bespoke helpers (candle puffs, burning-cross licks, and the rest of the inventory table) → `_fxDelay`, in helper-sized batches with regressions; then VFX-04 endpoint/list visibility. LIFE-06/VFX-03 are now closed for `_sigRun` refusal and asset attachment; they stay partial for the bespoke timers. Browser/host-guest acceptance remains open. This entry supersedes older next-task ordering.

### Latest continuation — 2026-09-10: LIFE-06 / VFX-03 shared helper cleanup

**Implemented and locally validated; not deployed or browser-playtested.** `_sigShockRing3D` and `_sigSpeedBurst3D` now dispose their instance geometry/materials when `_sigRun` refuses registration because of the active cap or a missing scene. Optional torus cleanup is included. Successful registration retains the existing animation/retirement owner, and shared textures remain intact. The shared `_sigRun` contract is unchanged.

**Flame scene ownership:** `spawnFlameBurst3D` and `_tickFlameBursts` previously disposed old-scene materials with scene removal explicitly skipped. They now detach old flame groups from their actual parent before disposing materials. This prevents disposed meshes remaining attached to a retained scene. Shared flame box geometry stays cached; normal re-ignition, expiration and clear behavior are preserved. The three audited helpers contain no additional delayed emission timers. Both viewers use these local helpers; no relay or authoritative gameplay changes.

**Validation:** 13 production-function regressions pass; eight fail against the unchanged baseline and five normal-behavior controls pass before and after. Coverage includes cap refusal, scene loss between admission and registration, torus-off geometry, animation completion, repeated cleanup, stale animation callbacks, flame scene switches through spawn/tick, re-ignition and expiration. Tests use controlled rendering objects and a stub flame-volume builder; they do not establish WebGL or live host/guest acceptance. Full package test command via bundled Node (`node --test *.test.js`, npm unavailable): **449 total, 445 passed, 2 previously recorded failures, 2 skips**. Failures remain the Phase 6 starter assertion and lunar-lander ceiling assertion. Syntax: **85/85 clean**. This accumulated local workspace is not full-main CI.

**Source and delivery:** GitHub main blob hashes for effects, index, CLAUDE.md and plan matched local copies before editing. Effects baseline blob: `270572d975e8faffcba7349bfe4b8daf3f2cffc5`. Complete files and validation evidence are in `ENTROPY_WARS_SHARED_HELPER_FIXES.zip`. three-vfx-effects.js → R2; index.html → Render; test, inventory, CLAUDE.md and plan → repository only. Cache token: `20260910-shared-helper-cleanup-01-cors`. No synced reference edits, commit, push or deployment.

**Exact next task:** continue the transitive wall geometry audit from `_fireWall` and its geometry dispatch, tracing delayed emissions and refused allocations. Then inspect asynchronous asset attachment while preserving application cache warmup, followed by VFX-04 endpoint/list visibility. Other `_sigRun` consumers still need refusal cleanup review. LIFE-06/VFX-03 remain partial; VFX-04 and browser/host-guest acceptance remain open. This entry supersedes older next-task ordering.

### Latest continuation — 2026-09-10: LIFE-06 / VFX-03 breath and boomerang retirement

**Implemented and locally validated; not deployed or browser-playtested.** Thirteen direct emission timer sites now use `_fxDelay`: breath charge/release, tongue waves, tile washes, terminal blast and smoke; boomerang outward rings, turnaround, return dash, reverse rings and catch; transitive dash crescents/dust and sonic-boom rings. Retirement cancels pending work and rejects queued callbacks after a fresh cast, including partially completed releases, turns and returns. Shared dash/sonic helpers confer the same protection on other callers. Existing geometry animation remains under `_sigRun`.

**Additional cleanup:** breath cones, dash crescents and sonic rings dispose their newly allocated geometry/materials if `_sigRun` refuses registration at its active cap or without a scene. This is local caller cleanup; the shared `_sigRun` contract is unchanged because other callers already handle refusal. Shared textures are not disposed. Other consumers' cap cleanup still needs review.

**Validation:** 16 production-function regression tests pass; 12 fail against unchanged source and four normal/suppression controls pass before and after. Tests cover fire/water payloads and timing, retirement before/after nested work, stale callbacks after a fresh cast, actual dash/sonic helper scheduling, normal geometry completion, exactly-once cleanup and allocation refusal. Rendering objects are controlled test doubles; these are not WebGL or network acceptance tests. Full package test command via bundled Node (`node --test *.test.js`, npm unavailable): **436 total, 432 passed, 2 previously recorded failures, 2 skips**. Failures remain the Phase 6 starter assertion and lunar-lander ceiling assertion. Syntax: **84/84 clean**. This accumulated local workspace is not full-main CI.

**Source and delivery:** GitHub main blob hashes for effects, index, CLAUDE.md and plan matched local files before editing. Effects blob: `3dfffa5d21a6af283764350bed4de2408eb16b3d`. Complete files, baseline hashes and validation logs are in `ENTROPY_WARS_BREATH_BOOMERANG_FIXES.zip`. three-vfx-effects.js → R2; index.html → Render; test, inventory and plan → repository only. Shared token: `20260910-breath-boomerang-retirement-01-cors`. Both online viewers execute these same local helpers; relay payloads and authority are unchanged. No synced reference edits, commit, push or deployment.

**Exact next task:** trace remaining downstream geometry owners (`spawnFlameBurst3D`, `_sigShockRing3D`, `_sigSpeedBurst3D`) and transitive wall geometry for callback ownership and refused-allocation cleanup. Then inspect asynchronous asset attachment while preserving shared cache warmup, followed by VFX-04 endpoint/list visibility. Do not infer complete transitive coverage from these four tested helpers. LIFE-06/VFX-03 and live acceptance remain open.
...
**Current continuation:** use the Aurora Curtain/Spiral Beam/Bad Trip entry at the top and ENTROPY_WARS_CONTINUED_VFX_FIXES.zip. This package includes the whiteout/seal/gas fixes. Next: Magic Circle, Magic Orb and Light Pillar refusal paths, then other return-only consumers, Psychosis emissions and VFX-04.
...
Phase log addition — 2026-09-10, LIFE-06/VFX-03: implemented 13 breath/boomerang and transitive dash/sonic emission timer sites, plus local geometry cleanup on refused registration. New regressions 16/16 (12 fail before, four controls pass); full suite 432 passed, two recorded failures, two skips; syntax 84/84. Complete-file package prepared, not deployed. Next: remaining downstream and wall geometry owners, asset attachment, then VFX-04.

Phase log addition — 2026-09-11, Phase 6 / AI-04a: refreshed main 82906801a763c0ce5fe943bdde48980d60ee7776; inventoried 501 spells / 65 kinds; repaired unreachable steal/cleanseArea handlers. Thirteen regressions pass, ten baseline failures; full suite 718 passed, zero failed, two skips. Complete files packaged locally; not deployed. Next: AI-04c prism self-cast correspondence, then wide beams.

Phase log addition — 2026-09-11, Phase 6 / AI-04c: prism self-target coordinates fixed; player-wide mirror ownership confirmed correct. Twelve checks pass, six fail before; full suite 730 passed, zero failed, two skips; syntax 105/105. Complete files packaged locally, not deployed. Next: AI-05 wide beams.

Phase log addition — 2026-09-11, Phase 6 / AI-05: wide-beam footprint and direction correspondence implemented across scoring, discovery, movement and ordinary/Simul re-aim. 28 new checks pass, 26 fail before; full suite 758 pass / 0 fail / 2 skips, syntax 106/106. Complete local files packaged, not deployed. Next: AI-04b guard/Trick Room and Tune valuation.

Phase log addition — 2026-09-11, Phase 6 / AI-04b: fixed Trick Room reset leakage; traced Chivalry transfer, Simul reversal and Tune/Pulse frequency gaps. 12 focused checks pass, five fail before; full suite 770 pass / 0 fail / 2 skips, syntax 108/108. Complete local delivery, not deployed. Next: Chivalry legality/net-transfer scoring, then Trick Room mode contract and coupled Tune/Pulse valuation.
